class ExportFamilyEnJob < ApplicationJob
  ...
  def render_member(member)
    ...
    personal_items = []
    personal_items << "<li><strong>Name:</strong> #{ERB::Util.html_escape(member.first_name)} #{ERB::Util.html_escape(member.last_name)}</li>"
    personal_items << "<li><strong>Birth Surname:</strong> #{ERB::Util.html_escape(member.birth_last_name)}</li>" if member.birth_last_name.present?
    personal_items << "<li><strong>Date of Birth:</strong> #{ERB::Util.html_escape(member.date_of_birth.strftime('%d.%m.%Y'))}</li>" if member.date_of_birth.present?
    personal_items << "<li><strong>Place of Birth:</strong> #{ERB::Util.html_escape(member.birth_place)}</li>" if member.birth_place.present?
    personal_items << "<li><strong>Time of Birth:</strong> #{ERB::Util.html_escape(member.birth_time.strftime('%H:%M'))}</li>" if member.birth_time.present?
    ...
    personal_items << "<li><strong>Religion:</strong> #{ERB::Util.html_escape(member.religion)}</li>" if member.religion.present?
    personal_items << "<li><strong>Profession:</strong> #{ERB::Util.html_escape(member.profession)}</li>" if member.profession.present?
    personal_items << "<li><strong>Hobbies and Interests:</strong> #{ERB::Util.html_escape(member.hobbies_and_interests)}</li>" if member.hobbies_and_interests.present?
    personal_items << "<li><strong>Description:</strong> #{ERB::Util.html_escape(member.short_description)}</li>" if member.short_description.present?
    personal_items << "<li><strong>Message:</strong> #{ERB::Util.html_escape(member.short_message)}</li>" if member.short_message.present?
    ...
    if member.deceased
      death_items = []
      death_items << "<li><strong>Date of Death:</strong> #{ERB::Util.html_escape(member.date_of_death.strftime('%d.%m.%Y'))}</li>" if member.date_of_death.present?
      death_items << "<li><strong>Time of Death:</strong> #{ERB::Util.html_escape(member.death_time.strftime('%H:%M'))}</li>" if member.death_time.present?
      death_items << "<li><strong>Place of Death:</strong> #{ERB::Util.html_escape(member.death_place)}</li>" if member.death_place.present?
      death_items << "<li><strong>Cause of Death:</strong> #{ERB::Util.html_escape(member.cause_of_death)}</li>" if member.cause_of_death.present?
      death_items << "<li><strong>Date of Burial:</strong> #{ERB::Util.html_escape(member.burial_date.strftime('%d.%m.%Y'))}</li>" if member.burial_date.present?
      death_items << "<li><strong>Place of Burial:</strong> #{ERB::Util.html_escape(member.burial_place)}</li>" if member.burial_place.present?
      death_items << "<li><strong>Place of Interment:</strong> #{ERB::Util.html_escape(member.internment_place)}</li>" if member.internment_place.present?
      if death_items.any?
        death_info = <<~HTML
          <h3>Death Details</h3>
          <ul>
    ...
    cover_html = <<~HTML
      <div class="cover">
        <div>
          ...
          #{ member.date_of_birth.present? ? "<p>Date of Birth: #{ERB::Util.html_escape(member.date_of_birth.strftime('%d.%m.%Y'))}</p>" : "" }
          #{ (member.deceased && member.date_of_death.present?) ? "<p>Date of Death: #{ERB::Util.html_escape(member.date_of_death.strftime('%d.%m.%Y'))}</p>" : "" }
        </div>
      </div>
    HTML
