class ExportFamilyMemberCsJob < ApplicationJob
  ...
  def perform(family_member_id)
    ...
    personal_items = []
    personal_items << "<li><strong>Jméno:</strong> #{ERB::Util.html_escape(family_member.first_name)} #{ERB::Util.html_escape(family_member.last_name)}</li>"
    personal_items << "<li><strong>Rodné příjmení:</strong> #{ERB::Util.html_escape(family_member.birth_last_name)}</li>" if family_member.birth_last_name.present?
    personal_items << "<li><strong>Datum narození:</strong> #{ERB::Util.html_escape(family_member.date_of_birth.strftime('%d.%m.%Y'))}</li>" if family_member.date_of_birth.present?
    personal_items << "<li><strong>Místo narození:</strong> #{ERB::Util.html_escape(family_member.birth_place)}</li>" if family_member.birth_place.present?
    personal_items << "<li><strong>Čas narození:</strong> #{ERB::Util.html_escape(family_member.birth_time.strftime('%H:%M'))}</li>" if family_member.birth_time.present?
    ...
    personal_items << "<li><strong>Víra:</strong> #{ERB::Util.html_escape(family_member.religion)}</li>" if family_member.religion.present?
    personal_items << "<li><strong>Povolání:</strong> #{ERB::Util.html_escape(family_member.profession)}</li>" if family_member.profession.present?
    personal_items << "<li><strong>Zájmy:</strong> #{ERB::Util.html_escape(family_member.hobbies_and_interests)}</li>" if family_member.hobbies_and_interests.present?
    personal_items << "<li><strong>Popis:</strong> #{ERB::Util.html_escape(family_member.short_description)}</li>" if family_member.short_description.present?
    personal_items << "<li><strong>Zpráva:</strong> #{ERB::Util.html_escape(family_member.short_message)}</li>" if family_member.short_message.present?
    ...
    if family_member.deceased
      death_items = []
      death_items << "<li><strong>Datum úmrtí:</strong> #{ERB::Util.html_escape(family_member.date_of_death.strftime('%d.%m.%Y'))}</li>" if family_member.date_of_death.present?
      death_items << "<li><strong>Čas úmrtí:</strong> #{ERB::Util.html_escape(family_member.death_time.strftime('%H:%M'))}</li>" if family_member.death_time.present?
      death_items << "<li><strong>Místo úmrtí:</strong> #{ERB::Util.html_escape(family_member.death_place)}</li>" if family_member.death_place.present?
      death_items << "<li><strong>Příčina úmrtí:</strong> #{ERB::Util.html_escape(family_member.cause_of_death)}</li>" if family_member.cause_of_death.present?
      death_items << "<li><strong>Datum pohřbu:</strong> #{ERB::Util.html_escape(family_member.burial_date.strftime('%d.%m.%Y'))}</li>" if family_member.burial_date.present?
      death_items << "<li><strong>Místo pohřbu:</strong> #{ERB::Util.html_escape(family_member.burial_place)}</li>" if family_member.burial_place.present?
      death_items << "<li><strong>Místo internace:</strong> #{ERB::Util.html_escape(family_member.internment_place)}</li>" if family_member.internment_place.present?
      if death_items.any?
        death_info = <<~HTML
          <h3>Údaje o úmrtí</h3>
          <ul>
    ...
    if children.any?
      children_html = "<h3>Potomci</h3>" + children.map do |child|
        ...
        content = "<p><strong>Jméno:</strong> #{ERB::Util.html_escape(child.first_name)} #{ERB::Util.html_escape(child.last_name)}</p>"
        content << "<p><strong>Datum narození:</strong> #{li_date}</p>" if li_date
        "<div class='card'>#{content}</div>"
      end.join
    end
